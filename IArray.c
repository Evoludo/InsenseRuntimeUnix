...
void *array_loc(IArrayPNTR a, unsigned i, bool *success){
  ...
  // this is not safe when e.g. trying to access array of structs
	if (i < a->length && i >= 0){
		...
		*success = false;// flag up error
		DAL_error(INDEX_OUT_OF_BOUNDS_ERROR);
#if DEBUG
		printf(" array:%p of:%i length:%i index:%i ", a, a->type_size, a->length, i);
#endif
...
static void IArray_decRef(IArrayPNTR pntr){
  if(DAL_mem_contains_pointers(pntr->data)){
    ...
    for(i=0; i<length; i++)
      DAL_decRef(((void **) pntr->data)[i]); // array deref code
